...
function WhyUs() {
    ...
    const reasons = React.useMemo<Reason[]>(() => t("why_us.reasons", {returnObjects: true}) as Reason[], [t]);

    console.log(reasons);

    return (<Container className="flex flex-col min-[1400px]:items-center">
            <header>
          ...
          {t("why_us.whyTheyChoose")}
            ...
            <main>
                <ul className="flex gap-2 overflow-auto">
                    {reasons.map((reason, index) => (<li key={index} className="flex flex-col">
                            ...
                            <div
                                className="px-10 bg-[#2F7FAA] text-lg font-semibold rounded-full p-1 text-center text-white whitespace-nowrap">
                                {reason.title}
                            </div>
